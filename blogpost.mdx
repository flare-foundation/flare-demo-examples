---
title: How to state connector
slug: how-to-state-connector
date: March. 15. 2024
published: true
thumbnailImage: /thumbnails/thumbnail-SC.png
description: How to use the State Connector.
tags: [intermediate, state-connector, flare]
---
...
![CCCR phases diagram](/postsImages/CCCR.png)
...
async function prepareRequest() {
  ...
  const response = await fetch(`${ATTESTATION_URL}/verifier/btc/Payment/prepareRequest`, {
    method: 'POST',
    headers: {
      'X-API-KEY': API_KEY,
...
async function testAttestation(scRound, requestData) {
  ...
  const response = await fetch(`${process.env.ATTESTER_BASE}/attestation-client/api/proof/get-specific-proof`, {
    method: 'POST',
    headers: {
...
export async function checkMerkleProof(scRound: number) {
  ...
  const stateConnector = await ethers.getContractAt(flareLib.nameToAbi('IStateConnector', 'coston').data, flareLib.nameToAddress('StateConnector', 'coston'));
...
async function getPreparedResponse() {
  ...
  const response = await fetch(`${ATTESTATION_URL}/verifier/btc/Payment/prepareResponse`, {
    method: 'POST',
